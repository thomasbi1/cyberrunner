...
def parallel(agent, replay, logger, make_env, num_envs, args):
    ...
    workers.append(
        embodied.distr.Thread(
            actor, step, agent, replay, logger, args.actor_addr, episodes, args
        )
    )
    workers.append(
        embodied.distr.Thread(
            learner, step, agent, replay, logger, timer, episodes, args
        )
    )
    if num_envs == 1:
        workers.append(
...
def actor(step, agent, replay, logger, actor_addr, episodes, args):
    ...
    def callback(obs, env_addrs):
        ...
        # print("Compute act took: {}".format(time.time() - start))

        trans = {**obs, **act}
        for i, a in enumerate(env_addrs):
            tran = {k: v[i].copy() for k, v in trans.items()}
            ep_trans[a].append(tran.copy())
        ...
        for i, a in enumerate(env_addrs):
            ...
            for tran in ep_trans[a]:
                replay.add(tran.copy(), worker=a)
            del ep_trans[a]
            metrics.add(stats, prefix="stats")
            logger.add(metrics.result())
            episodes.increment()
            print("Episodes: {}".format(int(episodes)))
            # agent.sync()
...
def learner(step, agent, replay, logger, timer, episodes, args):
    ...
    while True:
        ...
        if should_sync(episodes):
            agent.sync()

        if should_log(episodes):
